...
DistanceRobotObjResult Manipulator::computeDistToObj(GeometricPrimitives *obj, VectorXd q,
                                                     ...
                                                     DistanceRobotObjResult *oldDistStruct, double tol, double h, double maxDist)
{
...
DistanceRobotAutoResult Manipulator::computeDistAuto(VectorXd q, Matrix4d customHtmWorldToBase,
                                                     DistanceRobotAutoResult *oldDistStruct, double tol, double h, double maxDist)
{
  ...
  for (int k = 0; k < indLink1.size(); k++)
  {
    ...
    if (maxDist >= 10000 || Utils::distanceAABB(listObjsCopy[ind1][indCol1], listObjsCopy[ind2][indCol2]) <= maxDist)
    {
      ...
      DistanceStruct ds =
          GeometricPrimitives::computeDist(listObjsCopy[ind1][indCol1], listObjsCopy[ind2][indCol2], pointColObj1, h, tol);
                                             ...
                                             Matrix4d customHtmWorldToBase)
{
  ...
  for (int i = 0; i < obstacles.size(); i++)
  {
    ...
    dlo = this->computeDistToObj(obstacles[i], q, customHtmWorldToBase, &dloOld, param.tol, param.h, param.maxDistAABB);
  ...
  if (param.considerAutoCollision)
  {
    ...
    dra = this->computeDistAuto(q, customHtmWorldToBase, &draOld, param.tol, param.h, param.maxDistAABB);
...
ConstControlResult Manipulator::accelConstControl(Matrix4d taskHtm, VectorXd qdot, vector<GeometricPrimitives *> obstacles,
                                                  ConstControlResult *oldControlStruct, AccelConstControlParam param, VectorXd q,
                                                  Matrix4d customHtmWorldToBase)
{
  ...
  for (int i = 0; i < obstacles.size(); i++)
  {
    dloOld = (oldControlStruct == NULL) ? NULL : oldControlStruct->distancesObjResult[i];
    dlo = this->computeDistToObj(obstacles[i], q, customHtmWorldToBase, &dloOld, param.tol, param.h, param.maxDistAABB);
    dloNext = this->computeDistToObj(obstacles[i], q + qdot * param.dt, customHtmWorldToBase, &dlo, param.tol, param.h, param.maxDistAABB);
  ...
  if (param.considerAutoCollision)
  {
    ...
    draOld = (oldControlStruct == NULL) ? NULL : oldControlStruct->distanceAutoResult;
    dra = this->computeDistAuto(q, customHtmWorldToBase, &draOld, param.tol, param.h, param.maxDistAABB);
    draNext = this->computeDistAuto(q + qdot * param.dt, customHtmWorldToBase, &dra, param.tol, param.h, param.maxDistAABB);
  ...
  // Create H and f matrices
  TaskResult tr = this->taskFunction(taskHtm, q, customHtmWorldToBase);
  TaskResult trNext = this->taskFunction(taskHtm, q + qdot * param.dt, customHtmWorldToBase);
