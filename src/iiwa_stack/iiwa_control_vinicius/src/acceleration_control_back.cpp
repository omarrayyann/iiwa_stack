...
int main(int argc, char** argv)
{
  ...
  htmTg.push_back(Utils::trn(0.5, -0.3, 0.7) * Utils::rotx(M_PI / 2));
  htmTg.push_back(Utils::trn(0.6, 0, 0.7) * Utils::rotx(M_PI / 2));
  htmTg.push_back(Utils::trn(0.4, 0, 0.7) * Utils::roty(M_PI / 2));
  htmTg.push_back(iiwa.fk().htmTool);
  ...
  ROS_INFO("MAIN LOOP STARTED (COMING BACK)!!!");
  ...
  while (ros::ok() && notEnded)
  {
    ...
    if (reachedZeroPosition)
    {
      ...
      if (histq.size() > 0 && histdotq.size() > 0)
      {
        ...
        for (int n = 0; n < N; n++)
        {
          ccr = robot->accelConstControl(htmTg[indhtmTg], qdotsim / fac, obstacles, NULL, param, qsim);
        ...
        else
        {
          ...
          TaskResult tr = iiwa.taskFunction(htmTg[indhtmTg], q);
          VectorXd r = tr.task;
          ...
          if (indhtmTg == 0)
          {
            solvedTask = ccr.taskResult.maxErrorPos < 0.001 && ccr.taskResult.maxErrorOri < 5;
          }
          else
          {
            solvedTask = ccr.taskResult.maxErrorPos < 0.06 && ccr.taskResult.maxErrorOri < 10;
          }
