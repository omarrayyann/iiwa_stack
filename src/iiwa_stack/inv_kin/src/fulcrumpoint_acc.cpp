...
public:
  ...
  VectorXd lastValue() { return data[data.size() - 1]; }
  double lastTime() { return timeStamp[timeStamp.size() - 1]; }
...
double PARAM_FILT_QDOTKUKA = 0.05;  // Cut frequencies above 20rad/s 0.05
...
bool setConfigSpeed(VectorXd qdot)
{
  ...
  if (speedWithinLimits)
  {
    ...
    ROS_INFO_STREAM("ERROR: Speed outside limits... not sending");
}

bool setConfigAcceleration(VectorXd qddot)
{
  setConfigSpeed(getConfigSpeed() + PARAM_DT * qddot);
}
...
VectorXd fulcrumPointControlAccel(VectorXd q, VectorXd qdot, Vector3d vd, Vector3d pf)
{
  // Algorithm Parameters
  ...
  FulcrumPointResult fpResult = g_manip.computeFulcrumPoint(pf, q);
  FulcrumPointResult fpResultNext = g_manip.computeFulcrumPoint(pf, q + PARAM_DT * qdot);

  Vector3d pe = fpResult.fkr.htmTool.block<3, 1>(0, 3);
  Vector3d ve = fpResult.fkr.jacTool.block<3, 7>(0, 0) * qdot;

  double ex = fpResult.fx;
  double ey = fpResult.fy;
  double dotex = (fpResult.jacfx * qdot)[0];
  double dotey = (fpResult.jacfy * qdot)[0];

  if (g_count % 100)
  {
    ROS_INFO_STREAM(dotex);
    ROS_INFO_STREAM(dotey);
  }

  MatrixXd A1 = Utils::matrixVertStack(fpResult.jacfx, fpResult.jacfy);
  VectorXd b1 = Utils::vectorVertStack(-Kp * ex - Kd * dotex, -Kp * ey - Kd * dotey);

  MatrixXd A2 = fpResult.fkr.jacTool.block<3, 7>(0, 0);

  VectorXd b2 = -Kv * (ve - vd);

  vector<MatrixXd> A = {A1, A2};
  vector<VectorXd> b = {b1, b2};

  VectorXd qddot = Utils::hierarchicalSolve(A, b, 0.0001);
...
void kukaCallJoints(const sensor_msgs::JointState msg)
{
  if (!PARAM_ISSIM)
  {
    ...
    {
      if (g_qTimeSeries.size() > 0)
      {
        VectorXd qdotkuka_unfilt = (g_qkuka - g_qTimeSeries.lastValue()) / (getTime() - g_qTimeSeries.lastTime());
        double alpha = PARAM_FILT_QDOTKUKA / (PARAM_FILT_QDOTKUKA + PARAM_DT);
        g_qdotkuka = alpha * g_qdotkuka + (1 - alpha) * qdotkuka_unfilt;
      }
      else
        g_qdotkuka = VectorXd::Zero(7);
    }
  }

  if (g_reachedStartingPosition)
  {
    g_qTimeSeries.add(g_qkuka, getTime());
    g_qdotTimeSeries.add(g_qdotkuka, getTime());
  }
...
void printToFile()
{
  ...
  g_fileDebug << "qdot = [];" << std::endl;
  for (int i = 0; i < g_qdotTimeSeries.size(); i++)
    g_fileDebug << "qdot = [qdot; " << g_qdotTimeSeries.print(i) << "];" << std::endl;
...
TwoTimeSeries generatePath(VectorXd pd0, double t0, double maxtime)
{
  ...
  {
    desp.add(pd, t);
    vlin_des << 0, 0.015 * sin(2 * 3.14 * t / 6), 0;
    pd = pd + dt * vlin_des;
    VectorXd qdot = fulcrumPointControl(q, pd, PARAM_FP);
...
int main(int argc, char* argv[])
{
  ...
  while (ros::ok())
  {
    ...
    {
      Vector3d vd;
      vd << 0, 0 * 0.015 * sin(2 * 3.14 * (getTime()) / 6), 0;

      VectorXd qddot = fulcrumPointControlAccel(getConfig(), getConfigSpeed(), vd, PARAM_FP);

      setConfigAcceleration(qddot);
