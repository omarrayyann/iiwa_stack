...
struct DistanceRobotObjParam
{
  Matrix4d customHtmWorldToBase = Matrix4d::Zero();
  DistanceRobotObjResult *oldDistStruct = NULL;
  double tol = 0.0005;
  double h=0;
  double maxDist = 10000;
};

struct DistanceRobotAutoParam
{
  Matrix4d customHtmWorldToBase = Matrix4d::Zero();
  DistanceRobotAutoResult *oldDistStruct = NULL;
  double tol = 0.0005;
  double h=0;
  double maxDist = 10000;
};
...
public:
  ...
  static Manipulator createKukaIIWAReduced();

  static Manipulator createKukaIIWAWithTool();

  static Manipulator createKukaIIWAWithTool2();
  ...
  DistanceRobotObjResult computeDistToObj(GeometricPrimitives *obj, VectorXd q = VectorXd(0), DistanceRobotObjParam param = DistanceRobotObjParam());

  DistanceRobotAutoResult computeDistAuto(VectorXd q = VectorXd(0), DistanceRobotAutoParam param = DistanceRobotAutoParam());
