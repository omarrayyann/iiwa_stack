...
Manipulator Manipulator::createKukaIIWA()
{
  ...
  // Create links
  double d_bs = 0.360; //0.36
  double d_se = 0.420; //0.42
  double d_ew = 0.400; 
  double d_wf = 0.152;
...
Manipulator Manipulator::createKukaIIWAReduced()
{
  // Create empty manipulator
  Manipulator manip(7);
  ...
  // For the third link
  Cylinder *iiwaC2_0 = new Cylinder(Utils::trn(0, 0, 0.65), 0.08, 0.11);
  manip.links[2].colObjs.push_back(iiwaC2_0);
  manip.links[2].htmCols.push_back(fkres0.htmDH[2].inverse() * (*iiwaC2_0).htm);
  manip.links[2].nameObjs.push_back("c2_0");
  ...
  // For the fourth link
  Sphere *iiwaC3_0 = new Sphere(Utils::trn(0, -0.035, 0.803), 0.09);
  manip.links[3].colObjs.push_back(iiwaC3_0);
  manip.links[3].htmCols.push_back(fkres0.htmDH[3].inverse() * (*iiwaC3_0).htm);
  manip.links[3].nameObjs.push_back("c3_0");
  ...
  Box *iiwaC4_1 = new Box(Utils::trn(0, -0.08, 1.15), 0.12, 0.04, 0.17);
  manip.links[4].colObjs.push_back(iiwaC4_1);
  manip.links[4].htmCols.push_back(fkres0.htmDH[4].inverse() * (*iiwaC4_1).htm);
  manip.links[4].nameObjs.push_back("c4_1");

  // For the sixth link
  Cylinder *iiwaC5_0 = new Cylinder(Utils::trn(0, 0, 1.203), 0.075, 0.24);
  manip.links[5].colObjs.push_back(iiwaC5_0);
  manip.links[5].htmCols.push_back(fkres0.htmDH[5].inverse() * (*iiwaC5_0).htm);
  manip.links[5].nameObjs.push_back("c5_0");

  // For the seventh link
  //Box *iiwaC6_0 = new Box(Utils::trn(0.1, 0,  1.53), 0.20, 0.1, 0.46); 
...
Manipulator Manipulator::createKukaIIWAWithTool()
{
  ...
  // For the first link
  Cylinder *iiwaC0_0 = new Cylinder(Utils::trn(0, 0, 0.17), 0.1, 0.16);
  manip.links[0].colObjs.push_back(iiwaC0_0);
  manip.links[0].htmCols.push_back(fkres0.htmDH[0].inverse() * (*iiwaC0_0).htm);
  manip.links[0].nameObjs.push_back("c0_0");

  Sphere *iiwaC0_1 = new Sphere(Utils::trn(0.01, 0, 0.33), 0.125);
  manip.links[0].colObjs.push_back(iiwaC0_1);
  manip.links[0].htmCols.push_back(fkres0.htmDH[0].inverse() * (*iiwaC0_1).htm);
  manip.links[0].nameObjs.push_back("c0_1");

  // For the second link
  Sphere *iiwaC1_0 = new Sphere(Utils::trn(0, 0.03, 0.36), 0.11);
  manip.links[1].colObjs.push_back(iiwaC1_0);
  manip.links[1].htmCols.push_back(fkres0.htmDH[1].inverse() * (*iiwaC1_0).htm);
  manip.links[1].nameObjs.push_back("c1_0");

  Cylinder *iiwaC1_1 = new Cylinder(Utils::trn(0, 0, 0.53), 0.08, 0.126);
  manip.links[1].colObjs.push_back(iiwaC1_1);
  manip.links[1].htmCols.push_back(fkres0.htmDH[1].inverse() * (*iiwaC1_1).htm);
  manip.links[1].nameObjs.push_back("c1_1");
...
Manipulator Manipulator::createKukaIIWAWithTool2()
{
  // Create empty manipulator
  Manipulator manip(7);
  ...
  // For the seventh link
  Cylinder *iiwaC6_0 = new Cylinder(Utils::trn(0, 0,  1.53), 0.01, 0.4); 
  manip.links[6].colObjs.push_back(iiwaC6_0);
  manip.links[6].htmCols.push_back(fkres0.htmDH[6].inverse() * (*iiwaC6_0).htm);
  manip.links[6].nameObjs.push_back("c6_0");
...
DistanceRobotObjResult Manipulator::computeDistToObj(GeometricPrimitives *obj, VectorXd q, DistanceRobotObjParam param)
{
  ...
  for (int i = 0; i < this->noJoints; i++)
  {
    for (int j = 0; j < this->links[i].colObjs.size(); j++)
    {
      if (param.maxDist >= 10000 || Utils::distanceAABB(obj, listObjsCopy[i][j]) <= param.maxDist)
      {
        if (param.oldDistStruct != NULL)
        {
          dlo = (*param.oldDistStruct).getItem(i, j);
          if (dlo != NULL)
            pointObj = (*dlo).witnessObj;
          else
            pointObj = obj->htm.block<3,1>(0,3);
        }
        else
          pointObj = obj->htm.block<3,1>(0,3);

        DistanceStruct ds = GeometricPrimitives::computeDist(obj, listObjsCopy[i][j], pointObj, param.h, param.tol);
...
DistanceRobotAutoResult Manipulator::computeDistAuto(VectorXd q, DistanceRobotAutoParam param)
{
  ...
  for (int k = 0; k < indLink1.size(); k++)
  {
    ...
    if (param.maxDist >= 10000 || Utils::distanceAABB(listObjsCopy[ind1][indCol1], listObjsCopy[ind2][indCol2]) <= param.maxDist)
    {
      if (param.oldDistStruct != NULL)
      {
        dll = (*param.oldDistStruct).getItem(ind1, indCol1, ind2, indCol2);
        if (dll != NULL)
          pointColObj1 = (*dll).witnessColObjLink1;
        else
          pointColObj1 = listObjsCopy[ind1][indCol1]->htm.block<3,1>(0,3);
      }
      else
        pointColObj1 = listObjsCopy[ind1][indCol1]->htm.block<3,1>(0,3);

      DistanceStruct ds =
          GeometricPrimitives::computeDist(listObjsCopy[ind1][indCol1], listObjsCopy[ind2][indCol2], pointColObj1, param.h, param.tol);
...
ConstControlResult Manipulator::velocityConstControl(VectorXd q, VelocityConstControlParam param)
{
  ...
  for (int i = 0; i < param.obstacles.size(); i++)
  {
    ...
    paramdro.customHtmWorldToBase = param.customHtmWorldToBase;
    paramdro.oldDistStruct = &dloOld;
    paramdro.tol = param.tol;
    paramdro.h = param.h;
    paramdro.maxDist = param.maxDistAABB;
    dlo = this->computeDistToObj(param.obstacles[i], q, paramdro);
  ...
  if (param.considerAutoCollision)
  {
    ...
    DistanceRobotAutoParam paramdra;
    paramdra.customHtmWorldToBase = param.customHtmWorldToBase;
    paramdra.oldDistStruct = &draOld;
    paramdra.tol = param.tol;
    paramdra.h = param.h;
    paramdra.maxDist = param.maxDistAABB;
    dra = this->computeDistAuto(q, paramdra);
...
ConstControlResult Manipulator::accelerationConstControl(VectorXd qdot, VectorXd q, AccelerationConstControlParam param)
{
  ...
  for (int i = 0; i < param.obstacles.size(); i++)
  {
    ...
    paramdro.oldDistStruct = &dlo;
    dloNext = this->computeDistToObj(param.obstacles[i], q + qdot * param.dt, paramdro);
  ...
  if (param.considerAutoCollision)
  {
    ...
    paramdra.oldDistStruct = &dra;
    draNext = this->computeDistAuto(q + qdot * param.dt, paramdra);
    ...
    for (int j = 0; j < dra.noElements; j++)
    {
      ...
      DistanceLinkLinkResult *dllrNext = draNext.getItem(dra[j].linkNumber1, dra[j].colObjLinkNumber1, dra[j].linkNumber2, dra[j].colObjLinkNumber2);
      if (dllrNext != NULL)
      {
  ...
  VectorXd f = 2 * (tr.jacTask.transpose() * (coriolisCentrifugal + 2 * K * rdot + K * K * tr.task) + param.beta * qdot);
  ...
  while (fcr.isFree && i < this->noJoints)
  {
    ...
    while (fcr.isFree && j < this->links[i].colObjs.size())
    {
      ...
      while (fcr.isFree && k < param.obstacles.size())
      {
        ...
        if ((!ignoreCol) && (param.maxDistAABB >= 10000 || Utils::distanceAABB(param.obstacles[k], listObjsCopy[i][j]) <= param.maxDistAABB))
        {
          DistanceStruct ds =
              GeometricPrimitives::computeDist(param.obstacles[k], listObjsCopy[i][j], Vector3d::Random(), param.h, param.tol);
