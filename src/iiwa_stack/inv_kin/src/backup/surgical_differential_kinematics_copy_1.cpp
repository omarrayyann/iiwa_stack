...
FulcrumPointControlResult computeJointVelocitiesKuka4(Manipulator iiwa, VectorXd q_kuka, Vector3d p_des,
                                                      Vector3d vlin_des, Vector3d pf)
{
...
FulcrumPointControlResult computeJointVelocitiesKuka3(Manipulator iiwa, VectorXd q_kuka, Vector3d vlin_des, Vector3d pf)
{
  ...
  {
    ROS_INFO_STREAM("Safe mode!");
    qdot = Utils::pinv(A1, 0.001) * b1;  // 0.01
  }
  ...
  while (ros::ok() && notOver)
  {
    if (reachedStartingPosition)
    {
      if (touchEEfVelocitites.size() > 1)
      {
        ...
        FulcrumPointControlResult fpcr = computeJointVelocitiesKuka3(iiwa, q_kuka, vlin_des, fp);
