...
int main(int argc, char* argv[])
{
  ...
  while (ros::ok())
  {
    ...
    if (g_readedJoints && !reachedStartingPosition)
    {
      ...
      {
        ROS_INFO_STREAM("Generating data");
        tstart = getTime();
        qpath = generatePath();
        tend = getTime();
        ROS_INFO_STREAM("Done generating data!");
      ...
      g_taskTimeSeries.add(ccr.taskResult.task, getTime());
    ...
    if (g_readedJoints && reachedStartingPosition)
    {
      ...
      setConfig(qpath.atTime(getTime() - (tend - tstart)));
