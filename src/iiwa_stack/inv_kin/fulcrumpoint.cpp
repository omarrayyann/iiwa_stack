...
public:
  ...
  {
    if (t < timeStamp[0]) return data[0];
...
TimeSeries generatePath(double t0, double maxtime)
{
  VectorXd q = getConfig();
  double t = t0;
  VectorXd pd = g_pd;
...
int main(int argc, char* argv[])
{
  ...
  while (ros::ok())
  {
    ...
    if (g_readedJoints && !reachedStartingPosition)
    {
    ...
    if (g_readedJoints && reachedStartingPosition)
    {
      ...
      {
        g_targetq = generatePath(getTime(), PARAM_TIME);
      }
      else
      {
        setConfig(g_targetq.atTime(getTime()));
      }
      ...
      g_pdTimeSeries.add(g_pd, getTime());
